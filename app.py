...
APPS_SCRIPT_URL = ""   # paste your Apps Script /exec URL here to log to Sheet
...
(() => {
  ...
  function doSubmit() {
    ...
    // Optional: log to Google Sheet (with screenshot if premium)
    ...
    if (SHEET_URL && SHEET_URL.startsWith('http')) {
      try {
        fetch(SHEET_URL, {
          ...
          body: JSON.stringify({...data, screenshotBase64: uploadedScreenshot || ''})
        });
      } catch(e) { console.warn(e); }
    }

    // Local backup (without screenshot to save space)
    try {
